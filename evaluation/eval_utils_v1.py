...
def compute_degree_cm_mAP(final_results, synset_names, log_dir, degree_thresholds=[360], shift_thresholds=[100], iou_3d_thresholds=[0.1], iou_pose_thres=0.1, use_matches_for_pose=False, eval_recon=False, plot_figure=True):
    ...
    overlaps: [pred_boxes, gt_boxes] IoU overlaps.
    计算在特定交并比（IoU）阈值（默认通常是0.5）下的平均精度（Average Precision，简称AP）。IOU预测边界框与对应的真实边界框之间的交并比，是一个阈值。低于这个就认为不准确。
    返回结果包括：
    平均精度（mAP）：这是对特定类别或所有类别上精度-召回率曲线的平均处理，用来评估目标检测或语义分割任务中模型的表现。较高的mAP表示更好的检测性能。
    精确度（Precisions）：在每个阈值处的精确度是指真阳性（正确检测）数量除以真阳性与假阳性（误报）数量之和。你会得到一个随着类别得分阈值变化的精确度值列表，体现了不同决策阈值下的精确程度。
    召回率（Recalls）：每个阈值处的召回率是真阳性数量除以实际阳性（即真实存在的目标对象）总数。它反映了模型识别出所有应检测到目标的能力。同精确度一样，这也会生成一个随阈值变化的召回率值列表。
    重叠度（[pred_boxes, gt_boxes]的IoU）：这里的重叠指的是预测边界框（pred_boxes）和真实边界框（gt_boxes）之间的交并比（Intersection over Union）。通过计算它们重叠区域的面积与并集区域面积的比值来评估预测框与真实目标位置的匹配程度。
    在特定IoU阈值下计算mAP的过程一般涉及以下步骤：
    计算IoU：为每一对预测边界框和真实边界框计算IoU。
    阈值判断：根据IoU分数，确定哪些预测被判定为真阳性、假阳性或假阴性，基于给定的IoU阈值。
    精度-召回率曲线：对于预测的置信度分数的不同值（或是预设的一系列阈值），计算对应的精度和召回率，形成精度-召回率曲线。
    平均精度（Average Precision, AP）：计算召回率各级别的精度平均值，通常通过对等于或高于当前召回率级别的曲线上最大精度值进行取值来实现。
    平均平均精度（mAP）：如果评估多个类别，进一步对所有类别的平均精度（AP）值求平均，得到最终的mAP。
    这一过程全面地展示了模型在目标定位与分类上的准确性，同时考虑到了检测的精确性和全面性。
    """
    # log_dir就是outputpath
