...
# 数据处理。。。—_—
        ...
        img_list = []
        subset_len = []
        #  aggregate all availabel datasets 遍历每一个元素 对文件中的每一行确定正确的图像路径。去掉每一行的结尾处的\n
        ...
        # 冒号代表键值对，字典这种数据结构允许通过键（如 1 或者 2 等等）快速访问对应的值（如 'bottle' 或者 'bowl' 等等）。
        # 这个字典的功能可能是将分类的 ID 转换为其对应的类别名称。
    ...
    def __getitem__(self, index):
        ...
        data_dict['sym_info'] = torch.as_tensor(sym_infos.astype(float32)).contiguous()
        data_dict['mean_shape'] = torch.as_tensor(mean_shapes, dtype=torch.float32).contiguous()
        data_dict['pcl_in'] = torch.as_tensor(point_clouds.astype(float32)).contiguous()
    ...
    def generate_aug_parameters(self, s_x=(0.8, 1.2), s_y=(0.8, 1.2), s_z=(0.8, 1.2), ax=50, ay=50, az=50, a=15):
        ...
        return np.array([ex, ey, ez], dtype=float32), np.array([dx, dy, dz], dtype=float32) / 1000.0, Rm
